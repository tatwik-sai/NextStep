...
import { FaUserCheck, FaBriefcase, FaClipboardList } from "react-icons/fa6";
...
const MentorLayout = ({ children }) => {
    ...
    { name: "Dashboard", path: "dashboards", icon: <IoBarChart /> },
  ];
                ...
                <nav className="flex-1 p-4">
                    ...
                    return (
                        <Link
                            key={item.name}
                            href={item.path}
                            className={`flex items-center p-3 rounded mb-2 ${SIDEBAR_HOVER} ${isActive
